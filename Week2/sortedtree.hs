...
insertSorted :: Int -> Tree -> Tree
insertSorted x Leaf = Node x Leaf Leaf
insertSorted x (Node y l r) = 
  if x <= y
    then Node y (insertSorted x l) r
    else Node y l (insertSorted x r)
    
convertToList :: Tree -> [Int]
convertToList Leaf = []
convertToList (Node x l r) = convertToList l ++ [x] ++ convertToList r
